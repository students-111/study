...
> **auto-embedded 工具技能**：脚本随框架装在 `.auto-embedded/tools/debug-platformio/`，用 `py` 运行 `.auto-embedded/tools/debug-platformio/scripts/` 下脚本；详细用法见 `.auto-embedded/tools/debug-platformio/references/usage.md`。
...
2. 探测调试环境：
   ```bash
   py .auto-embedded/tools/debug-platformio/scripts/pio_debugger.py --detect --project-dir <工程目录> --env <环境名>
   ```
3. 执行调试：
   ```bash
   py .auto-embedded/tools/debug-platformio/scripts/pio_debugger.py --project-dir <工程目录> --env <环境名> --mode download-and-halt
   ```
