...
> **auto-embedded 工具技能**：脚本随框架装在 `.auto-embedded/tools/flash-keil/`，用 `py` 运行 `.auto-embedded/tools/flash-keil/scripts/` 下脚本；详细用法见 `.auto-embedded/tools/flash-keil/references/usage.md`。
...
2. 对于常见场景，直接一次调用完成探测+烧录：
   ```bash
   py .auto-embedded/tools/flash-keil/scripts/keil_flasher.py --detect --flash --project <工程文件> --target <目标名>
   ```
