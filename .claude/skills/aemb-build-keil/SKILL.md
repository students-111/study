...
> **auto-embedded 工具技能**：脚本随框架装在 `.auto-embedded/tools/build-keil/`，用 `py` 运行 `.auto-embedded/tools/build-keil/scripts/` 下脚本；详细用法见 `.auto-embedded/tools/build-keil/references/usage.md`。
...
2. 对于单工程、单目标的常见场景，优先使用一次调用完成探测+编译：
   ```bash
   py .auto-embedded/tools/build-keil/scripts/keil_builder.py --detect --project <工程文件> --target <目标名>
   ```
