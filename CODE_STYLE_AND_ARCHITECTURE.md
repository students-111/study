...
- `dal_jy901p` 通过 `bsp_i2c` 异步读取 JY901P（兼容官方 JY61P 寄存器协议）在 `0x3D` 起始的 6 字节 Roll/Pitch/Yaw 已解算角度帧；直接以小端 `int16_t` 原始寄存器值发布 `roll_raw`、`pitch_raw`、`yaw_raw`，不在 MCU 侧进行单位换算、Kalman、角速度积分或姿态解算。JY901P 短暂 I2C 异常时保留最近有效快照，超过 `DAL_JY901P_SNAPSHOT_STALE_TIMEOUT_MS` 未收到有效帧才清除 `angle_reference_ready`；对超出 `DAL_JY901P_YAW_MAX_STEP_RAW` 的单帧航向跳变直接丢弃。首帧读取完成后，`g_dal_jy901p_sample.angle_reference_ready` 置位，APP 才能消费 `yaw_raw`；如将 `DAL_JY901P_REFERENCE_RESET_ENABLE` 设为 `1U`，还需等待官方参考归零流程结束。
...
- APP 读取 DAL 快照，例如 `g_dal_gray_sample`、`g_dal_jy901p_sample`。
